...
# Purpose Of Script: This script transfers data between databases and fills data gaps in the specified time intervals for the Fascia system.
# Testing Phase: Phase 1

# Log file path
LOG_FILE="/tmp/Db-To-Db-error_log1.txt"
...
WHERE last_check BETWEEN UNIX_TIMESTAMP(DATE_FORMAT(CURDATE(), '%Y-%m-%d 00:00:00'))
      AND UNIX_TIMESTAMP(DATE_FORMAT(CURDATE(), '%Y-%m-%d 12:00:00'))"
...
SELECT
    last_state_change,
    UNIX_TIMESTAMP(DATE_FORMAT(CURDATE(), '%Y-%m-%d 00:00:00')) AS last_check,
